...
/**
  ...
  */
uint16_t Class_crclite::Get_CRC16_Check_Sum(const uint8_t * pchMessage, uint32_t dwLength, uint16_t wCRC)
{
  uint8_t ch_data;
...
bool Class_crclite::Verify_CRC16_Check_Sum(const uint8_t * pchMessage, uint32_t dwLength)
{
  uint16_t w_expected = 0;

  if ((pchMessage == NULL) || (dwLength <= 2)) return false;

  w_expected = Class_crclite::Get_CRC16_Check_Sum(pchMessage, dwLength - 2, CRC16_INIT);
  return (
...
@return none
*/
void Class_crclite::Append_CRC16_Check_Sum(uint8_t * pchMessage, uint32_t dwLength)
{
  uint16_t w_crc = 0;

  if ((pchMessage == NULL) || (dwLength <= 2)) return;

  w_crc = Class_crclite::Get_CRC16_Check_Sum(pchMessage, dwLength - 2, CRC16_INIT);
