 ...
 */
void can_filter_mask_config(CAN_HandleTypeDef *hcan1, uint8_t Object_Para, uint32_t ID, uint32_t Mask_ID)
{
	
    //检测传参是否正确
    assert_param(hcan1 != NULL);
    ...
    //使能滤波器
    can_filter_init_structure.FilterActivation = DISABLE;

    // 过滤器配置
    if(HAL_CAN_ConfigFilter(hcan1, &can_filter_init_structure)!=HAL_OK)
    {
        Error_Handler();
    }
 ...
 */
void CAN_Init(CAN_HandleTypeDef *hcan1, CAN_Call_Back Callback_Function)
{
    if (hcan1->Instance == CAN1)
    {
        CAN1_Manage_Object.CAN_Handler = hcan1;
        CAN1_Manage_Object.Callback_Function = Callback_Function;					
//         can_filter_mask_config(hcan1, CAN_FILTER(0) | CAN_FIFO_0 | CAN_STDID | CAN_DATA_TYPE, 0x200 ,0x7F8);  //只接收0x200-0x207
//         can_filter_mask_config(hcan1, CAN_FILTER(1) | CAN_FIFO_1 | CAN_STDID | CAN_DATA_TYPE, 0x200, 0x7F8);
			can_filter_mask_config(hcan1, CAN_FILTER(0) | CAN_FIFO_0 | CAN_STDID | CAN_DATA_TYPE, 0 ,0);
			can_filter_mask_config(hcan1, CAN_FILTER(1) | CAN_FIFO_1 | CAN_STDID | CAN_DATA_TYPE, 0 ,0);
    }
    ...
    /*开中断*/
    HAL_CAN_ActivateNotification(hcan1, CAN_IT_RX_FIFO0_MSG_PENDING);       //can 接收fifo 0不为空中断
	HAL_CAN_ActivateNotification(hcan1, CAN_IT_RX_FIFO1_MSG_PENDING);       //can 接收fifo 1不为空中断
}
 ...
 * @return uint8_t 执行状态
 */
uint8_t CAN_Send_Data(CAN_HandleTypeDef *hcan1, uint16_t ID, uint8_t *Data, uint16_t Length)
{
    ...
    return (HAL_CAN_AddTxMessage(hcan1, &tx_header, Data, &used_mailbox));
}

/**
 * @brief CAN1的TIM定时器中断发送回调函数
 *
 */
extern uint16_t CAN_ID;
extern CAN_HandleTypeDef hcan1;
static uint8_t cyccntcan1=0;
void TIM_CAN_PeriodElapsedCallback()
{

	cyccntcan1++;
	  if(cyccntcan1>=4){
    CAN_Send_Data(&hcan1, CAN_ID, CAN1_0x200_Tx_Data, 8);
			cyccntcan1=0;
		}
...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan1)
{
    ...
    if (hcan1->Instance == CAN1)
    {
        HAL_CAN_GetRxMessage(hcan1, CAN_FILTER_FIFO0, &CAN1_Manage_Object.Rx_Buffer.Header, CAN1_Manage_Object.Rx_Buffer.Data);
...
void HAL_CAN_RxFifo1MsgPendingCallback(CAN_HandleTypeDef *hcan1)
{
    ...
    if (hcan1->Instance == CAN1)
    {
        HAL_CAN_GetRxMessage(hcan1, CAN_FILTER_FIFO1, &CAN1_Manage_Object.Rx_Buffer.Header, CAN1_Manage_Object.Rx_Buffer.Data);
        CAN1_Manage_Object.Callback_Function(&CAN1_Manage_Object.Rx_Buffer);
    }
